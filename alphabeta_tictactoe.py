...
def minimax_alpha_beta(board):
    ...
    if current_player == X:
        ...
        for action in actions(board):
            val = min_value_alpha_beta(result(board, action), alpha, beta)
            if val > best_val:
                best_val = val
                best_action = action
        ...
        for action in actions(board):
            val = max_value_alpha_beta(result(board, action), alpha, beta)
            if val < best_val:
                best_val = val
                best_action = action
...
def max_value_alpha_beta(board, alpha, beta):
    """
    Returns the maximum utility value for X using alpha-beta pruning.
    """
    if terminal(board):
    ...
    for action in actions(board):
        v = max(v, min_value_alpha_beta(result(board, action), alpha, beta))
        # Pruning: if v >= beta, we can stop exploring this branch
        if v >= beta:
            return v
        # Update alpha
        alpha = max(alpha, v)
...
def min_value_alpha_beta(board, alpha, beta):
    """
    Returns the minimum utility value for O using alpha-beta pruning.
    """
    if terminal(board):
        return utility(board)
    ...
    for action in actions(board):
        v = min(v, max_value_alpha_beta(result(board, action), alpha, beta))
        # Pruning: if v <= alpha, we can stop exploring this branch
        if v <= alpha:
            return v
        # Update beta
        beta = min(beta, v)
...
def play_game():
    ...
    while not terminal(board):
        ...
        # Computer's turn (O)
            ...
            end_time = time.time()
            print(f"Computer took {end_time - start_time:.4f} seconds to decide.")
            board = result(board, action)
...
def play_tic_tac_toe_simulation():
    """
    Simulates a full game of Tic Tac Toe with both players using alpha-beta pruning.
    """
    board = initial_state()
    print("Initial board:")
    print_board(board)
